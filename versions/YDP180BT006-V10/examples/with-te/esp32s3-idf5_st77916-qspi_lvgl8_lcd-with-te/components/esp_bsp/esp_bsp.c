...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_st77916(*ret_io, &panel_config, ret_panel));
...
esp_err_t bsp_touch_new(const bsp_display_cfg_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    esp_lcd_touch_config_t tp_cfg = {
        .x_max = EXAMPLE_LCD_SPI_H_RES,
        .y_max = EXAMPLE_LCD_SPI_V_RES,
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_touch_new_i2c_cst816s(tp_io_handle, &tp_cfg, &tp_handle), TAG, "New cst816 failed");
